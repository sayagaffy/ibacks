...
export default async function Home() {
  // Parallel fetching from CMS & WMS Cache
  const [heroBanners, cacheData] = await Promise.all([
  ...
  const rawProducts = cacheData?.products || [];
  const products = rawProducts.slice(0, 8);

  // Dynamically filter products for those with `isPromo=true`
  const promoProducts: PromoProduct[] = rawProducts
    .filter(p => p.isPromo)
    .map((p) => {
      const image = p.thumbnail || p.variants?.find(v => v.thumbnail)?.thumbnail || 'https://images.unsplash.com/photo-1592890288564-76628a30a657?q=80&w=800';
      return {
        id: p.id,
        name: p.name,
        category: p.categoryId ? `Kategori ${p.categoryId}` : 'Aksesoris', // Since category name is separate, fallback to generic
        price: p.price,
                ...
                return (
                  <Link key={p.id} href={`/products/${p.id}`}>
                    <ProductCard
                      ...
                      imageSrc={image}
                      category={p.categoryId ? `Kategori ${p.categoryId}` : 'Aksesoris'}
                    />
                  </Link>
                );
              })
